...
(function () {
  ...
  function waMessage(r) {
    ...
    const axisName = AX()[r.main_axis] || '';
    const lines = [
      `السلام عليكم ورحمة الله.. مساء الخير يا ${name || ''}،`,
      '',
      'ممتنّين لإكمالك رحلة "هندسة العقلية" — وده مش وقت قليل، ده التزام حقيقي مع نفسك.',
      '',
      'من اختياراتك خلال الرحلة، طلعت بصمتك:',
      `• البصمة: *${r.fingerprint_name || '—'}*`,
      `• محورك الرئيسي: *${axisName}*`,
      `• بابك ونكهتك: ${DR()[r.door] || '—'} · ${FL()[r.flavor] || '—'}`,
      `• نوع احتراقك المُرشّح: ${BR()[r.burnout_type] || '—'}`,
      '',
      'جهّزتلك بصمتك الكاملة في الرابط ده — تفتحها وتحفظها وقت ما تحب:',
      resultUrl(r),
      '',
      `واحتفظ بالكود ده، هتحتاجه لو حبّيت تفتح تقريرك تاني: ${r.result_code || ''}`,
      ''
    ];
    if (CONFIG.courseUrl) {
      lines.push('— — —', '', 'ودي كانت بداية. العمق الحقيقي والـ٢٧ بصمة بتفاصيلها والمسارات بنفتحها في الدورة الكاملة:', CONFIG.courseUrl, '');
    } else {
      lines.push('— — —', '', 'ودي كانت بداية التشخيص. العمق الحقيقي والـ٢٧ بصمة بتفاصيلها والمسارات بنفتحها في الدورة الكاملة قريبًا.', '');
    }
    lines.push('— محمود فؤاد', 'منظور الفؤاد');
    return lines.join('\n');
  }
