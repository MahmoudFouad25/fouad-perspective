 /* ============================================================================
   🧮  محرّك الحاسبة + الحجوزات + الكوبونات — ريتريت "المحراب"
   ============================================================================
...
(function () {
  ...
  /* =========================================================================
     1) Firebase initialization (نفس مشروع fouad-perspective)
     ========================================================================= */
  let db = null;
  let fbReady = false;
    ...
    if (offer) {
      ...
      <div class="offers-row primary-offer-row">
        <div class="offer-card price-card urgent">
          ...
          <div class="active-until"><i class="fas fa-bolt"></i><span>عرض الإطلاق محدود — السعر يعود لأصله بعد انتهائه</span></div>
        </div>
      </div>`;
    } else {
      html += `
      <div class="offers-row primary-offer-row">
        <div class="offer-card price-card">
          ...
          <div class="offer-header"><h4>الاستثمار في الريتريت</h4><i class="fas fa-moon offer-icon"></i></div>
  ...
  async function applyCoupon() {
    ...
    input.disabled = true;
    input.style.background = "rgba(46,196,182,.12)";
    ...
    document.getElementById("calcCouponMessage").innerHTML =
      `<div style="background:rgba(46,196,182,.1);border:1px solid var(--teal);padding:10px;border-radius:8px;color:var(--teal-bright);margin-top:10px;"><i class="fas fa-check-circle"></i> الكود <strong>${validCoupon.code}</strong> مُفعّل ${validCoupon.type === "percentage" ? `(خصم ${validCoupon.value}%)` : `(خصم ${validCoupon.value} ${cur})`}</div>`;
  ...
  function swal(icon, title, text, timer) {
    const opts = { icon, title, confirmButtonColor: "#1e8a90" };
  ...
  function showBookingForm() {
    ...
    Swal.fire({
      ...
      html: `<div style="text-align:right;direction:rtl;max-height:500px;overflow-y:auto;padding-left:8px;">
        <div style="background:linear-gradient(135deg,#1e8a90,#14595d);padding:14px;border-radius:10px;margin-bottom:18px;color:#fff;">
          <h4 style="margin:0;font-family:Amiri,serif;"><i class="fas fa-moon"></i> ${d.courseTitle}</h4>
        ...
        <div style="background:#F0FDF4;padding:18px;border-radius:10px;margin-bottom:14px;">
          ...
          ${textarea("customerExpectations", "ما الذي تحمله معك إلى هذا الريتريت؟ (اختياري)")}
          ${field("customerExperience", "خبرة سابقة في العمل على الذات (اختياري)")}
  ...
  function textarea(id, label, rows) {
    return `<div style="margin-bottom:12px;"><label style="display:block;margin-bottom:5px;color:#374151;">${label}</label>
      <textarea id="${id}" rows="${rows || 3}" style="width:100%;padding:11px;border:2px solid #E5E7EB;border-radius:8px;resize:none;background:#fff;color:#000;font-family:Tajawal,sans-serif;"></textarea></div>`;
  }
  ...
  function showGrantForm() {
    ...
    window.__mhr_updateInstallment = function () {
    ...
    window.__mhr_updatePartial = function () {
      ...
      if (document.getElementById("installPartial")?.checked) window.__mhr_updatePartialInst();
    };
    window.__mhr_togglePartialInst = function () {
      ...
      if (c) window.__mhr_updatePartialInst();
    };
    window.__mhr_updatePartialInst = function () {
    ...
    window.__mhr_toggleSupport = function () {
      ...
      if (s === "installment" && id) { id.style.display = "block"; window.__mhr_updateInstallment(); }
      else if (s === "partial" && pd) { pd.style.display = "block"; window.__mhr_updatePartial(); }
    };
    ...
    Swal.fire({
      ...
      html: `<div style="text-align:right;direction:rtl;max-height:500px;overflow-y:auto;padding-left:8px;">
        ...
        <div style="background:#FFF7ED;padding:18px;border-radius:10px;margin-bottom:14px;border-right:4px solid #F59E0B;">
          ...
          ${textarea("communityContribution", "كيف ستساهم في مجتمعك بعد الرحلة؟ (اختياري)", 3)}
        </div>
        ...
        <div style="background:#F3E8FF;padding:18px;border-radius:10px;margin-bottom:14px;">
          ...
          <div style="margin-bottom:14px;">
            ...
            <select id="employmentStatus" style="width:100%;padding:11px;border:2px solid #E9D5FF;border-radius:8px;background:#fff;color:#000;font-family:Tajawal,sans-serif;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
            <div>
              ...
              <select id="hasIncome" style="width:100%;padding:11px;border:2px solid #E9D5FF;border-radius:8px;background:#fff;color:#000;font-family:Tajawal,sans-serif;">
          ...
          <div style="background:#fff;padding:16px;border-radius:10px;margin-bottom:12px;border:2px solid #E9D5FF;">
            <label style="display:flex;align-items:flex-start;cursor:pointer;">
              <input type="radio" name="supportType" value="installment" onchange="__mhr_toggleSupport()" style="margin-left:10px;margin-top:3px;width:18px;height:18px;">
              <div style="width:100%;">
                ...
                <div id="installmentDetails" style="display:none;margin-top:14px;">
                  ...
                  <input type="range" id="installmentMonths" min="2" max="18" value="3" oninput="__mhr_updateInstallment()" style="width:100%;margin:8px 0;">
          ...
          <div style="background:#fff;padding:16px;border-radius:10px;margin-bottom:12px;border:2px solid #E9D5FF;">
            <label style="display:flex;align-items:flex-start;cursor:pointer;">
              <input type="radio" name="supportType" value="partial" onchange="__mhr_toggleSupport()" style="margin-left:10px;margin-top:3px;width:18px;height:18px;">
              <div style="width:100%;">
                ...
                <div id="partialDetails" style="display:none;margin-top:14px;">
                  ...
                  <input type="range" id="affordableAmount" min="${minPay}" max="90" value="50" oninput="__mhr_updatePartial()" style="width:100%;margin:8px 0;">
                  <div style="text-align:center;padding:13px;background:#F3E8FF;border-radius:8px;">
                  ...
                  <div style="margin-top:14px;padding:13px;background:#FEF3C7;border-radius:8px;">
                    <label style="cursor:pointer;display:flex;align-items:center;gap:8px;color:#000;font-size:.92rem;">
                      <input type="checkbox" id="installPartial" onchange="__mhr_togglePartialInst()" style="width:17px;height:17px;">
                      أريد تقسيط هذا المبلغ على شهور
                    </label>
                    <div id="partialInstallmentDetails" style="display:none;margin-top:12px;">
                      <input type="range" id="partialInstallmentMonths" min="2" max="12" value="3" oninput="__mhr_updatePartialInst()" style="width:100%;">
          ...
          <div style="background:#fff;padding:16px;border-radius:10px;border:2px solid #E9D5FF;">
            <label style="display:flex;align-items:flex-start;cursor:pointer;">
              <input type="radio" name="supportType" value="full" onchange="__mhr_toggleSupport()" style="margin-left:10px;margin-top:3px;width:18px;height:18px;">
              <div>
        ...
        <div style="background:#FEF2F2;padding:18px;border-radius:10px;margin-bottom:14px;">
          ...
          ${commit("commitment1", "أتعهّد بأن أعيش هذا الريتريت بصدق وأستثمر محتواه بجدّية", true)}
          ...
          <div style="margin-top:14px;padding:13px;background:#F0FDF4;border-radius:8px;">
            ${commit("volunteerCommitment", "أتطوّع بمساعدة ٣ أشخاص من مجتمعي بعد إتمام الرحلة (اختياري)", false)}
          </div>
        </div>
      ...
      didOpen: () => {
        ...
        window.__mhr_updateInstallment();
        window.__mhr_updatePartial();
      },
      preConfirm: () => {
  ...
  async function saveBookingAndOpenTelegram(bookingData) {
    ...
    const telegramCode = ("MH" + Math.random().toString(36).slice(2, 8) + Date.now().toString(36).slice(-4)).toUpperCase();

    const fullRecord = {
      ...bookingData,
      bookingId: "MH_" + Date.now() + "_" + Math.random().toString(36).slice(2, 11),
