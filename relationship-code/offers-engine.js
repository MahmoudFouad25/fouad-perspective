/* ============================================================================
   🧮  محرّك الحاسبة + الحجوزات + الكوبونات — دورة "شفرة العلاقات"
   ============================================================================
...
(function () {
  ...
  /* =========================================================================
     3) عرض بطاقات العروض (السيكشن الترويجي)
     ========================================================================= */
    ...
    if (offer) {
      ...
      <div class="offers-row primary-offer-row">
        <div class="offer-card price-card urgent">
          ...
          <div class="offer-header">
            <h4>استثمارك في العلاقة</h4><i class="fas fa-heart offer-icon"></i>
          </div>
      ...
      <div class="offers-row primary-offer-row">
        <div class="offer-card price-card">
          ...
          <div class="offer-header"><h4>الاستثمار في الدورة</h4><i class="fas fa-heart offer-icon"></i></div>
  ...
  async function applyCoupon() {
    ...
    input.disabled = true;
    input.style.background = "rgba(193,123,116,.1)";
    ...
    document.getElementById("calcCouponMessage").innerHTML =
      `<div style="background:rgba(193,123,116,.08);border:1px solid var(--rose);padding:10px;border-radius:8px;color:var(--rose-bright);margin-top:10px;"><i class="fas fa-check-circle"></i> الكود <strong>${validCoupon.code}</strong> مُفعّل ${validCoupon.type === "percentage" ? `(خصم ${validCoupon.value}%)` : `(خصم ${validCoupon.value} ${cur})`}</div>`;
  ...
  function swal(icon, title, text, timer) {
    const opts = { icon, title, confirmButtonColor: "#B5734C" };
  ...
  function showBookingForm() {
    ...
    Swal.fire({
      ...
      html: `<div style="text-align:right;direction:rtl;max-height:500px;overflow-y:auto;padding-left:8px;">
        <div style="background:linear-gradient(135deg,#B5734C,#8A5436);padding:14px;border-radius:10px;margin-bottom:18px;color:#fff;">
          <h4 style="margin:0;font-family:Amiri,serif;"><i class="fas fa-heart"></i> ${d.courseTitle}</h4>
        ...
        <div style="background:#FDF2F2;padding:18px;border-radius:10px;margin-bottom:14px;">
          <h4 style="color:#9D3B43;margin-bottom:14px;font-family:Amiri,serif;"><i class="fas fa-comments"></i> سياق علاقتك (اختياري)</h4>
          ${select("relContext", "أكتر علاقة عايز تشتغل عليها", ["", "علاقة زوجية", "علاقة مع أحد الأبناء", "علاقة أسرية/أهل", "علاقة عمل/فريق", "صداقة", "علاقاتي بشكل عام", "أخرى"])}
          ${select("attendMode", "هتخوض الرحلة إزاي؟", ["", "بمفردي", "أنا وشريكي (كوبل)", "ضمن مجموعة", "ككوتش/مدرّب أستخدمها مع عملائي"])}
          ${textarea("relExpectations", "إيه اللي تتمنى يتغيّر في علاقتك؟", 2)}
        </div>
      ...
      const customer = {
        ...
        relContext:   document.getElementById("relContext").value || "",
        attendMode:   document.getElementById("attendMode").value || "",
        expectations: document.getElementById("relExpectations").value.trim() || "",
  ...
  function select(id, label, options) {
    const opts = options.map((o) => `<option value="${o}">${o === "" ? "-- اختر --" : o}</option>`).join("");
    return `<div style="margin-bottom:12px;"><label style="display:block;margin-bottom:5px;color:#374151;">${label}</label>
      <select id="${id}" style="width:100%;padding:11px;border:2px solid #E5E7EB;border-radius:8px;background:#fff;color:#000;font-family:Cairo,sans-serif;">${opts}</select></div>`;
  }
  ...
  function showGrantForm() {
    ...
    window.__rc_updateInstallment = function () {
    ...
    window.__rc_updatePartial = function () {
      ...
      if (document.getElementById("installPartial")?.checked) window.__rc_updatePartialInst();
    };
    window.__rc_togglePartialInst = function () {
      ...
      if (c) window.__rc_updatePartialInst();
    };
    window.__rc_updatePartialInst = function () {
    ...
    window.__rc_toggleSupport = function () {
      ...
      if (s === "installment" && id) { id.style.display = "block"; window.__rc_updateInstallment(); }
      else if (s === "partial" && pd) { pd.style.display = "block"; window.__rc_updatePartial(); }
    };

    Swal.fire({
      title: "🤲 طلب منحة دراسية / دعم مالي",
      html: `<div style="text-align:right;direction:rtl;max-height:500px;overflow-y:auto;padding-left:8px;">
        <div style="background:linear-gradient(135deg,#9D3B43,#7C2D34);padding:15px;border-radius:10px;margin-bottom:18px;color:#fff;">
          <h4 style="margin:0;font-family:Amiri,serif;"><i class="fas fa-heart"></i> ${d.courseTitle}</h4>
        </div>
        <div style="background:rgba(157,59,67,0.08);padding:18px;border-radius:10px;margin-bottom:18px;text-align:center;">
          <i class="fas fa-hand-holding-heart" style="font-size:2.2rem;color:#9D3B43;"></i>
          <h3 style="color:#7C2D34;margin:10px 0 4px;font-family:Amiri,serif;">المال لا يجب أن يكون عائقاً</h3>
          <p style="color:#9D3B43;margin:0;font-size:.92rem;">طلبك يُعامَل بسرّية تامّة، وبتفهّم لظروفك.</p>
        </div>
        ...
        <div style="background:#FFF7ED;padding:18px;border-radius:10px;margin-bottom:14px;border-right:4px solid #F59E0B;">
          ...
          ${textarea("communityContribution", "كيف ستساهم في علاقاتك ومجتمعك بعد التعلّم؟ (اختياري)", 3)}
        </div>

        <div style="background:#FDF2F8;padding:18px;border-radius:10px;margin-bottom:14px;">
          <h4 style="color:#9D174D;margin-bottom:14px;font-family:Amiri,serif;"><i class="fas fa-hand-holding-heart"></i> الدعم المطلوب</h4>

          <div style="margin-bottom:14px;">
            <label style="font-weight:600;color:#9D174D;display:block;margin-bottom:6px;">الوضع الوظيفي <span style="color:red;">*</span></label>
            <select id="employmentStatus" style="width:100%;padding:11px;border:2px solid #FBCFE8;border-radius:8px;background:#fff;color:#000;font-family:Cairo,sans-serif;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
            <div>
              <label style="color:#9D174D;display:block;margin-bottom:6px;">دخل ثابت؟</label>
              <select id="hasIncome" style="width:100%;padding:11px;border:2px solid #FBCFE8;border-radius:8px;background:#fff;color:#000;font-family:Cairo,sans-serif;">
            ...
            <div>
              <label style="color:#9D174D;display:block;margin-bottom:6px;">عدد المعالين</label>
              <input type="number" id="dependents" min="0" style="width:100%;padding:11px;border:2px solid #FBCFE8;border-radius:8px;background:#fff;color:#000;">
            </div>
          </div>

          <label style="font-weight:600;color:#9D174D;display:block;margin-bottom:10px;">نوع الدعم المطلوب <span style="color:red;">*</span></label>

          <!-- التقسيط المُيسر -->
          <div style="background:#fff;padding:16px;border-radius:10px;margin-bottom:12px;border:2px solid #FBCFE8;">
            <label style="display:flex;align-items:flex-start;cursor:pointer;">
              <input type="radio" name="supportType" value="installment" onchange="__rc_toggleSupport()" style="margin-left:10px;margin-top:3px;width:18px;height:18px;">
              <div style="width:100%;">
                <strong style="color:#9D174D;">تقسيط مُيسّر على شهور أكتر</strong>
                ...
                <div id="installmentDetails" style="display:none;margin-top:14px;">
                  <label style="color:#9D174D;font-size:.88rem;">عدد الأشهر:</label>
                  <input type="range" id="installmentMonths" min="2" max="18" value="3" oninput="__rc_updateInstallment()" style="width:100%;margin:8px 0;">
                  <div style="text-align:center;padding:13px;background:#FDF2F8;border-radius:8px;">
                    <div style="font-size:1.1rem;color:#9D174D;font-weight:700;"><span id="monthsCount">3</span> أشهر</div>
                    <div style="font-size:1.4rem;color:#B5734C;font-weight:700;margin-top:4px;"><span id="monthlyAmount">0</span> ${cur} / شهرياً</div>
                  </div>
                </div>
              </div>
            </label>
          ...
          <div style="background:#fff;padding:16px;border-radius:10px;margin-bottom:12px;border:2px solid #FBCFE8;">
            <label style="display:flex;align-items:flex-start;cursor:pointer;">
              <input type="radio" name="supportType" value="partial" onchange="__rc_toggleSupport()" style="margin-left:10px;margin-top:3px;width:18px;height:18px;">
              <div style="width:100%;">
                <strong style="color:#9D174D;">منحة جزئية — أقدر أدفع جزء فقط</strong>
                ...
                <div id="partialDetails" style="display:none;margin-top:14px;">
                  <label style="color:#9D174D;font-size:.88rem;">النسبة اللي تقدر تدفعها:</label>
                  <input type="range" id="affordableAmount" min="${minPay}" max="90" value="50" oninput="__rc_updatePartial()" style="width:100%;margin:8px 0;">
                  <div style="text-align:center;padding:13px;background:#FDF2F8;border-radius:8px;">
                    <div style="color:#9D174D;font-size:.95rem;">تدفع <strong><span id="percentageAmount">50</span>%</strong> من المبلغ</div>
                    <div style="font-size:1.4rem;color:#B5734C;font-weight:700;margin-top:4px;"><span id="finalAmount">0</span> ${cur}</div>
                  </div>
                  <div style="margin-top:14px;padding:13px;background:#FEF3C7;border-radius:8px;">
                    <label style="cursor:pointer;display:flex;align-items:center;gap:8px;color:#000;font-size:.92rem;">
                      <input type="checkbox" id="installPartial" onchange="__rc_togglePartialInst()" style="width:17px;height:17px;">
                      أريد تقسيط هذا المبلغ على شهور
                    </label>
                    <div id="partialInstallmentDetails" style="display:none;margin-top:12px;">
                      <input type="range" id="partialInstallmentMonths" min="2" max="12" value="3" oninput="__rc_updatePartialInst()" style="width:100%;">
          ...
          <!-- منحة كاملة -->
          <div style="background:#fff;padding:16px;border-radius:10px;border:2px solid #FBCFE8;">
            <label style="display:flex;align-items:flex-start;cursor:pointer;">
              <input type="radio" name="supportType" value="full" onchange="__rc_toggleSupport()" style="margin-left:10px;margin-top:3px;width:18px;height:18px;">
              <div>
                <strong style="color:#9D174D;">منحة كاملة ١٠٠٪</strong>
        ...
        <div style="background:#FEF2F2;padding:18px;border-radius:10px;margin-bottom:14px;">
          ...
          ${commit("commitment3", "المعلومات التي أدخلتها صحيحة وحقيقية")}
          <div style="margin-top:14px;padding:13px;background:#FDF2F8;border-radius:8px;">
            ${commit("volunteerCommitment", "أتطوّع بمساعدة ٣ أشخاص من محيطي على فهم علاقاتهم بعد إتمام الدورة (اختياري)")}
          </div>
        </div>
      ...
      cancelButtonText: "رجوع",
      confirmButtonColor: "#9D3B43",
      width: window.innerWidth <= 768 ? "95%" : "780px",
      allowOutsideClick: false,
      didOpen: () => {
        document.getElementById("customerName")?.focus();
        window.__rc_updateInstallment();
        window.__rc_updatePartial();
      },
      preConfirm: () => {
  ...
  function commit(id, label) {
    return `<div style="margin-bottom:8px;"><label style="cursor:pointer;display:flex;align-items:flex-start;gap:8px;color:#000;font-size:.92rem;line-height:1.6;">
      <input type="checkbox" id="${id}" style="margin-top:3px;width:17px;height:17px;flex-shrink:0;"><span>${label}</span>
    </label></div>`;
  }

  /* =========================================================================
     9) حفظ الحجز في Firebase + تليجرام تلقائي
     ========================================================================= */
  async function saveBookingAndOpenTelegram(bookingData) {
    ...
    const telegramCode = ("RC" + Math.random().toString(36).slice(2, 8) + Date.now().toString(36).slice(-4)).toUpperCase();

    const fullRecord = {
      ...bookingData,
      bookingId: "RC_" + Date.now() + "_" + Math.random().toString(36).slice(2, 11),
    ...
    await Swal.fire({
      ...
      html: `<div style="text-align:right;direction:rtl;">
        ...
        <div style="background:#F0F9FF;border:1px solid #BAE6FD;border-radius:12px;padding:18px;margin-bottom:14px;">
          ...
          <p style="color:#1E293B;margin:0 0 6px;font-size:.95rem;line-height:1.75;">١. اضغط الزر الأزرق أسفل هذه النافذة.</p>
          <p style="color:#1E293B;margin:0 0 6px;font-size:.95rem;line-height:1.75;">٢. في تليجرام، اضغط زر <strong style="color:#0EA5E9;">Start</strong>.</p>
          <p style="color:#1E293B;margin:0;font-size:.95rem;line-height:1.75;">٣. هنرسل لك تعليمات الدفع وتفاصيل التأكيد فوراً.</p>
        </div>
